...
public class UpdateCultivoActivity extends AppCompatActivity {
    ...
    public void actualizarCultivo(){
        ...
        final Cultivo cultivo = new Cultivo(id, nombre.getText().toString(), Integer.parseInt(tempMax.getText().toString()), Integer.parseInt(tempMin.getText().toString()), fechaInicial.getText().toString(),fechaFinal.getText().toString(), false);
    ...
    public void activeCultivo(){

        progress.setTitle("Activando...");

        AlertDialog alertDialog = new AlertDialog.Builder(UpdateCultivoActivity.this).create();
        alertDialog.setTitle("Confirmación");
        alertDialog.setMessage("Está seguro que desea activar el cultivo?");
        alertDialog.setButton(AlertDialog.BUTTON_POSITIVE, "Aceptar",
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        progress.show();
                        ...
                        Call<BasicResponse> activeCultivo = webServices.activeCultivo(id);

                        activeCultivo.enqueue(new Callback<BasicResponse>() {
                            @Override
                            public void onResponse(Call<BasicResponse> call, Response<BasicResponse> response) {
                                progress.dismiss();
                                if (response.code() == 200) {
                                    BasicResponse activoCultivo = response.body();

                                    if(activoCultivo.getSuccess()){
                                        onSupportNavigateUp();
                                    }
                                    else {
                                        showError("Ocurrió un problema al conectarse con el servidor, por favor intenta nuevamente.");
